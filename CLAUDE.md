# Sauna Log Project Guide

## 1. Project Overview
- **Goal**: 3-Click 기반 사우나/찜질방 기록 및 인스타그램 스토리 공유 PWA
- **Key Feature**: 사용자 그룹(목욕파/사우너파/찜질파)별 맞춤형 로그 템플릿과 서치 가능한 데이터베이스 제공
- **Target User**: 쉽고 예쁘게 사우나 생활을 기록하고 공유하고 싶은 2030 유저들

## 2. Tech Stack
- **Frontend**: Next.js 14+, TypeScript, Tailwind CSS
- **Backend/DB**: Supabase (Auth, Storage 포함)
- **Deployment**: Vercel & PWA (Future: Capacitor for App)

## 3. Development Rules
- **Coding Style**: 초보자도 이해 가능하도록 주석을 달고, 단계별로 설명할 것.
- **Naming**: 파일 및 폴더명은 소문자 기반 케밥 케이스(kebab-case) 권장.
- **UI/UX**: 모바일 웹(PWA) 우선 디자인, 기능적인 메뉴나 버튼에는 'Google Material Symbols'를 사용하고, 시각적인 포인트나 감성적인 요소(먹거리, 상태 등)에는 '이모지(Emoji)'를 사용하여 컬러감을 살릴 것.

## 4. Metadata for AI
- 이 프로젝트는 사우너 루틴과 리뷰 전문성을 반영하여 단순한 기록을 넘어 전문적인 리뷰 앱이 되도록 도와야 함
- 모든 에러 메시지는 해결 방법과 함께 영어로 친절하게 설명할 것.