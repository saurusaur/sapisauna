...
export default function UserLogCard({ log, onClick, showPlace = false, compact = false }: UserLogCardProps) {
  ...
  if (compact) {
    return (
      <Wrapper
        onClick={onClick}
        className={`w-full glass-card-light px-3 py-3.5 text-left h-[124px] flex flex-col justify-between overflow-hidden${onClick ? ' hover:shadow-md active:scale-[0.98] transition-all' : ''}`}
      >
        {/* Line 1: 장소명 + 트라이브 이모지 */}
  ...
  return (
    <Wrapper
      onClick={onClick}
      className={`w-full glass-card-light p-4 text-left${onClick ? ' hover:shadow-md active:scale-[0.98] transition-all' : ''}`}
    >
      {/* 장소명 */}
      {showPlace && (
