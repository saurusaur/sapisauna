...
**⚠️ 구조 변경됨 (2026-03-09):**
- `/story/edit` 및 커스텀 에디터 전부 삭제됨
- `/story`는 순수 뷰어/공유 도구로 전환
- DB 저장은 스토리 진입 전에 완료됨 (분기 모달에서)
- 하단 버튼: "한 번 더 기록" / "홈으로" / "내 기록" (Lovable과 유사)

#### 2-8. 기록 완료 (Log Complete.png) — ❌ 삭제됨
- `/complete` 페이지 삭제, 성공 토스트로 대체
- 스토리 진입 시 "기록이 저장되었어요!" 인라인 토스트 표시
- Lovable의 3단 버튼 구조는 스토리 페이지 하단에 통합됨
...
| # | 항목 | 결정 | 상태 |
|---|------|------|------|
| A1 | **Primary 컬러** | 그린 → 레드 `#cc1a1a` | ✅ 적용됨 (Phase 0) |
| A2 | **CTA 버튼** | 그린 → 레드 배경 | ✅ 적용됨 (Phase 0-1) |
| A3 | **점수/메트릭** | 오렌지 → 레드 | ✅ 적용됨 (Phase 0-1) |
| A4 | **타입별 컬러** | 파/주/초 유지 (맥락별 판단) | 🔍 페이지별 확인 중 |

### B. UI 패턴 변경

| # | 항목 | 결정 | 상태 |
|---|------|------|------|
| B1 | **토토노이/또갈래요** | 슬라이더 → Slider with steps (사실상 칩) | ✅ 구현됨 |
| B2 | **온보딩 타입 카드** | 세로 → 가로 | ⬜ Phase 8에서 적용 |
| B3 | **달력 뷰** | 월간 유지 (사용자 결정) | ✅ 유지 |
| B4 | **시설 칩** | 맥락별 판단 | 🔍 페이지별 확인 중 |
| B5 | **세신/매점 토글** | 스타일 변경 예정 | ⬜ Phase 4에서 적용 |

### C. 타이포그래피

| # | 항목 | 결정 | 상태 |
|---|------|------|------|
| C1 | **한글 폰트** | DM Sans + Noto Sans KR | ✅ 적용됨 (Phase 0) |
| C2 | **페이지 헤딩** | 영문 이탤릭 세리프 | ✅ 홈 적용됨, 나머지 Phase별 |
| C3 | **본문 weight** | 400 → 500 | ✅ 적용됨 (Phase 0) |
...
## 4. 글래스모픽 효과 적용 가이드 ✅ 구현됨

### CSS 유틸리티 (globals.css에 추가됨)
...
1. ~~디자인 토큰~~ ✅ 완료
2. ~~글래스 유틸리티~~ ✅ 완료
3. **화면별 적용** — ~~홈~~ ✅ → 퀵로그 → 탐색 → 딥로그 → 설정 → 온보딩 → 스토리 순
