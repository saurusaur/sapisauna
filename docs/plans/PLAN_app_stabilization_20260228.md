...
2. `src/middleware.ts:67` — `getUser()` try/catch 감싸기 + `/onboarding`을 publicRoutes에 추가
...
6. `src/app/login/page.tsx:12-18` — OAuth try/catch + 에러 UI
...
9. JSON.parse 호출 전수 교체 → `safeParse()` (grep으로 정확한 목록 확인 후 진행)

### P2: Lint 정리 (2파일)
10. `src/app/layout.tsx:37` — Google Fonts `<link>` → `next/font/google`
11. `src/app/history/page.tsx:100,116` — useMemo deps 보완
