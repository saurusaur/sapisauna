# 핸드오프: 로그 블록 컷오버 — 입력 폼 구현·디자인 다듬기 완료

작성 2026-06-07 / 최종갱신 2026-06-08 / 브랜치 **preview** (프로덕션 main 미머지) / **상태: 029 적용 + 코드 컷오버 1~4단계 + 입력 폼 디자인 다듬기 전부 완료·프리뷰 푸시. 잔여=5단계(표시면 평탄화)·030 cleanup·main 머지·스토리 연결.**
...
2. **5단계 표시면 평탄화 (미적용 — 중요)**: `story/page.tsx`+`image-export.ts`, `history/[id]`, `record-card`, `user-log-card`, `home-calendar`, `reward-service`, `explore/[id]`. 현재 `deep_log.x`·구 컬럼 읽음 → **새 폼으로 저장한 로그가 표시면에서 제대로 안 보임**(저장 자체는 됨). 1차는 평탄 캐시로 호환 → 블록 시퀀스 렌더는 점진.
...
**사-첵 완료** — 홈 로고(`/logo/sapi-chek-logo.svg`) **도장 버튼을 폼 맨 끝 인플로우**로 박음(플로팅 아님). 150px, **왼쪽 15도 틸트**, 살짝 우측. 비활성=`grayscale opacity-50`(무색), 활성=레드+입체 쉐도우. `canSave`=활동≥1 && 또갈래요 && (주사우나 필요시 선택).

**공통** — 모바일 입력 포커스 시 `scrollIntoView`(키보드 위), 토글 열 때 화면 **중앙**으로 스크롤. 간격=10/40 규칙, 폰트=타입 스케일(메모리 표준).

**변경 파일**: `src/app/log/page.tsx`, `src/components/slider.tsx`, `src/constants/content.ts` (전부 preview 푸시 완료, fb32f5b~). content.ts: 습식 온도범위 40→80, 노천탕 steps 5단계, 혼잡도 '혼잡', 평가 '아쉽'.
